...
\subsection{HTML}
Le pagine del sito sono state realizzate in \textbf{HTML5} e validate seguendo le buone pratiche di sviluppo web. In particolare:
\begin{itemize}
  \item è stata garantita una chiara \textbf{separazione tra struttura, presentazione e comportamento}, mantenendo distinti fogli di stile CSS, file PHP e script JavaScript dal codice HTML;
  \item sono stati definiti \textbf{meta tag} appropriati nella sezione \texttt{<head>} per migliorare SEO e accessibilità;
  \item per contenuti intrinsecamente tabellari (es. \textbf{orari di apertura} nel \texttt{footer}) sono state utilizzate \textbf{tabelle semantiche} con \texttt{<caption>}, \texttt{<thead>}/\texttt{<tbody>}, intestazioni \texttt{<th>} con \verb|scope="col"|/\verb|scope="row"| e descrizione associata tramite \verb|aria-describedby|, così da risultare pienamente comprensibili anche agli screen reader.
\end{itemize}
...
\begin{itemize}[leftmargin=2.2em]
  ...
  \item \texttt{password\_hash} — \textbf{CHAR(64)}, \textbf{NOT NULL}: hash della password (esadecimale, p.\,es.\ SHA--256).
   \item \texttt{username} — \textbf{VARCHAR(30)}, \textbf{NOT NULL}: credenziale di login.
...
\subsubsection{\texttt{product\_types}}
